...
function PrescriptionReturnThroughEntry({
  through,
  num,
  onClick,
  prescription,
  ...
  const { mutateAsync: prescriptionThroughPatch } = useMutation({
    mutationFn: (data) =>
      patchDataFn(data, `prescription-return-through/${through.id}/`),
    onSuccess: () => {
      successFn(
  ...
  const { mutateAsync: prescriptionThroughDelete } = useMutation({
    mutationFn: () => deleteDataFn(`prescription-return-through/${through.id}/`),
    onSuccess: () => {
      successFn(
        `prescription-return-through/?prescription=${prescription?.id}`,
        () => {
  ...
  const selectAutoCompleteData = (data) => {
    ...
    axios
      .patch(`${serverIP}api/prescription-return-through/${through.id}/`, Form)
      .then(() => {
        updatePrescriptionThrough();
...
export default PrescriptionReturnThroughEntry;
