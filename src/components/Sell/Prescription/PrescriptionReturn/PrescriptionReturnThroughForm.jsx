...
import AdditionalMedicineModalReturn from "./AdditionalMedicineModalReturn";

export const PrescriptionReturnThroughForm = forwardRef(
  ({ prescription, prescriptionThrough, updatePrescription }, ref) => {
    ...
    const { mutateAsync: prescriptionThroughPost } = useMutation({
      ...
      onSuccess: () => {
        successFn(
          `prescription-return-through/?prescription=${prescription?.id}`,
          () => {}
        );
      },
    });
    ...
    const { mutateAsync: prescriptionThroughPatch } = useMutation({
      mutationFn: (data) =>
        patchDataFn(
          data,
          `prescription-return-through/${medicineIncludesCheck().id}/`
        ),
      onSuccess: () => {
        successFn(
          `prescription-return-through/?prescription=${prescription?.id}`,
    ...
    return (
      <>
      <AdditionalMedicineModalReturn ref={additionalRef} medicine={medicine} prescriptionThrough={prescriptionThrough} prescription={prescription} medicineIncludesCheck={medicineIncludesCheck} medicineSelectRef={ref}>
      </AdditionalMedicineModalReturn>
        <AlertModal
