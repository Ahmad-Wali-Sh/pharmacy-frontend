...
import { useMedicineOpener, usePrescription, usePrescriptionReturn, useUserPermissions } from "../../../States/States";
...
import MultiplePrescriptionReturnImage from "../../../PageComponents/MultiplePrescriptionReturnImage";

function PrescriptionReturnForm({ prescriptionThrough, update, prescriptionSelected }) {
  const { userPremissions } = useUserPermissions();
  const user = useAuthUser();
  const { data: patient } = useQuery(["patient/"]);
  const { data: doctor } = useQuery(["doctor/"]);
  const { data: department } = useQuery(["department-return/"]);
  ...
  const newPrescription = () => {
    serverIP &&
      axios
        .get(`${serverIP}api/department-return/` + watch("department") + "/")
        .then((res) => {
  ...
  const { mutateAsync: updatePrescription } = useMutation({
    mutationFn: (data) => {
      patchDataFn(data, `prescription-return/${prescription.id}/`);
    },
    onSuccess: (data) =>
      successFn("", () => {
        update();
  ...
  const { mutateAsync: prescriptionThroughPost } = useMutation({
    mutationFn: (data) => postDataFn(data, "prescription-return-through/"),
    onSuccess: () => {
      successFn("", () => {});
    },
  });
  ...
  const { mutateAsync: duplicatePrescription } = useMutation({
    mutationFn: (data) => postDataFn(data, "prescription-return/"),
    onSuccess: (res) => {
      successFn("", () => {
        setPrescription(res);
  ...
  const { refetch: deletePrescription } = useQuery({
    queryKey: [`prescription-return-through/delete/?prescription=${prescription?.id}`],
    enabled: false,
    onSuccess: () => successFn("", () => {}),
  });
  ...
  const { refetch: prescriptionSearch } = useQuery({
    queryKey: ["prescription-return/?prescription_number=" + searchNumber],
    enabled: false,
    onSuccess: (data) => {
    ...
    <>
      ...
      {showPrescriptionForm ? (
        ...
        >
          ...
          />
          <label>عکس:</label>
          <MultiplePrescriptionReturnImage />
          <label>تخفیف:</label>
          <div className="flex">
...
export default PrescriptionReturnForm;
