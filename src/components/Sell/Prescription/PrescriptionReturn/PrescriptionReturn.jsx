...
import { usePrescription, usePrescriptionReturn, useUserPermissions } from "../../../States/States";
import useServerIP from "../../../services/ServerIP";
import NewMedicineListener from "../../../Settings/NewMedicineListener";
import PrescriptionReturnForm from "./PrescriptionReturnForm";
import PrescriptionReturnReportBox from "./PrescriptionReturnReportBox";
import { PrescriptionReturnThroughForm } from "./PrescriptionReturnThroughForm";
import PrescriptionReturnThroughMapFrom from "./PrescriptionReturnThroughMapFrom";

export default function PrescriptionReturn(props) {
  ...
  const { prescription, setPrescription } = usePrescriptionReturn();
  ...
  useEffect(() => {
    prescription?.department &&
      axios
        .get(`${serverIP}api/department-return/${prescription.department}/`)
        .then((res) => {
  ...
  const departmentSubmit = () => {
    ...
    axios
      .get(`${serverIP}api/department-return/` + props.department.id + "/")
      .then((res) => {
        ...
        axios
          .post(`${serverIP}api/prescription-return/`, DepartmentForm)
          .then((res) => {
  ...
  const handleSurfPrescription = (functionality) => {
    if (prescription?.id) {
      axios.get(`${serverIP}api/prescription-return/${prescription?.id}/${functionality}/`).then((res) => {
        setPrescription([])
      ...
      axios.get(`${serverIP}api/last-prescription-return/`).then((res) => {
        setPrescription([])
  ...
  const updatePrescription = () => {
    axios
      .get(`${serverIP}api/prescription-return/` + prescription.id + "/")
      .then((res) => {
        setPrescription(res.data)
        setTimeout(() => {
          Reporting()
  ...
  const { data: pres, refetch: updatePrescrip } = useQuery({
    queryKey: [`prescription-return/${prescription?.id}/`],
    onSuccess: (res) => {
      setPrescription(res);
    },
    enabled: prescription?.id != undefined,
  ...
  return (
    <>
      ...
      <BigModal title="ثبت برگشتی" ref={PrescriptionModalRef}>
        {loading ? (
          ...
          <div className="modal">
            ...
            <div className="prescription-box">
              ...
              <div className="prescription-details">
                ...
                />
                <PrescriptionReturnThroughForm
                  prescription={prescription}
                  prescriptionThrough={prescriptionThrough}
                  ref={SelectMedicineModalRef}
                />
                <PrescriptionReturnThroughMapFrom
                  prescription={prescription}
                  updatePrescription={updatePrescription}
