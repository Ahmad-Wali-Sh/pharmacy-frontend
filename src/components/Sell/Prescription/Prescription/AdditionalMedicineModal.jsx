...
const AdditionalMedicineModal = forwardRef(({ medicine, prescription, prescriptionThrough, medicineSelectRef }, ref) => {
  ...
  const onAdditionalSubmit = (e) => {
    e.preventDefault();
    const inputData = Object.entries(inputValues).map(([addinId, inputValue]) => {
        const addin = medicine.add_medicine[0].additional.find(a => a.id === parseInt(addinId));
        return {
          addinId,
