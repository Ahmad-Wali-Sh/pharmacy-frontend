...
import { usePrescription, usePrescriptionReturn } from "../States/States";
import SmallModal from "./Modals/SmallModal";
import WebCamModal from "./WebCamModal";
import useServerIP from "../services/ServerIP";


function MultiplePrescriptionReturnImage() {
  const MultipleImageRef = useRef(null);
  const { prescription, setPrescription } = usePrescriptionReturn()
  const { refetch: prescriptionRefetch } = useQuery(`prescription-return/${prescription?.id}/`, {
  ...
  const { mutate: newImage } = useMutation({
    mutationFn: () => {
      ...
      postDataFn(ImageForm, "prescription-return-image/");
    },
    onSuccess: () => {
      successFn("", () => {
        setTimeout(() => {
  ...
  const { mutate: deleteImage } = useMutation({
    mutationFn: (data) => {
      deleteDataFn(`prescription-return-image/${data?.id}/`);
    },
    onSuccess: () => {
      successFn("", () => {
        setTimeout(() => {
...
export default MultiplePrescriptionReturnImage;
