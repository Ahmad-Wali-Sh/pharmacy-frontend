...
export const SelectMedician = forwardRef(
  (
    ...
    let medicineQuery = `medician/?${
      ...
      stringArray[2] ? encodeURIComponent(stringArray[2]) : ""
    }&kind__name_persian=${
      ...
      stringArray[6] ? encodeURIComponent(stringArray[6]) : ""
    }&all=${stringArray[7] ? encodeURIComponent(stringArray[7]) : ""}`;

    let medicineBarcodeQuery = `medicine-barcode/?barcode=${
      stringArray[0] ? encodeURIComponent(stringArray[0]) : ""
    }`;

    const { data: MedicianSearched, isFetching: MedicineLoading } = useQuery({
      queryKey: [isBarcode(stringArray) ? medicineBarcodeQuery : medicineQuery],
      enabled: stringArray.join("").replace(/\s/g, "") != "",
    });

    const handleMedicineSelect = (item) => {
      MedicineShowRef.current.Opener(item);
      const newIndex = MedicianSearched?.results?.findIndex(
    ...
    const handleKeyDown = (event) => {
      if (
        event.key === "ArrowDown" &&
        currentIndex === MedicianSearched?.results?.length - 1
      ) {
        event.preventDefault(); // Prevent default behavior
      } else if (event.key === "ArrowDown") {
        setCurrentIndex((prevIndex) =>
          Math.min(prevIndex + 1, MedicianSearched?.results?.length - 1)
        );
            ...
            <BigModal title="انتخاب دارو" ref={SelectMedicineModalRef}>
              ...
              <div className="medician-select-input-box">
                <div>
                  ...
                  |{" "}
                  <span className={textHighlight?.kind_persian}>
                    نوع "فارسی"
                  </span>{" "}
